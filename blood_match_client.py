...
recipients = requests.get("http://vcm-7631.vm.duke.edu:5002"
                          "/get_patients/am937")
...
for iter in recipents_dict.keys():
    blood_type_get = requests.get(f"http://vcm-7631.vm.duke."
                                  "edu:5002/get_blood_type/"
                                  f"{recipents_dict[iter]}")
...
out_data = {"Name": 'am937',
            "Match": 'No'}
r_add_patient = requests.post("http://vcm-7631.vm.duke.edu:5002/match_check",
                              json=out_data)
